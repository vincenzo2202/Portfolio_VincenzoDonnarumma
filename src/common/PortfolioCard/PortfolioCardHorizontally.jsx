import './PortfolioCardHorizontally.css';

export const PortfolioCardHorizontally = ({ cardImage, cardName, cardInfo, cardPreview, cardCode, cardSkills }) => {
  return (
    <div className="portfolio-card-horizontal">
      <div className="portfolio-card__img-wrapper">
        <img className="portfolio-card__img-horizontally" src={cardImage} alt={cardName} />
      </div>
      <div className="portfolio-card__descr-wrapper">
        <div className='portfolio-card__descr-top'>
          <p className="portfolio-card__title">{cardName}</p>
          <div className='portfolio-card__descr-stack'>
            {cardSkills.map((skill, index) => (
              <div key={index} className='skills'>{skill}</div>
            ))}
          </div>
        </div>
        <p className="portfolio-card__descr">{cardInfo}</p>
        <div className="portfolio-card__links">
          <a className="portfolio-card__link" href={cardPreview} target="_blank" rel="noopener noreferrer">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" className="portfolio-card__svg">
          ...
          <a className="portfolio-card__link" href={cardCode} target="_blank" rel="noopener noreferrer">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 496 512" className="portfolio-card__svg">
